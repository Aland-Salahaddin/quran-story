  ...
  return (
    <div className="relative min-h-screen" style={{ background: "var(--bg)" }}>
      ...
      >
        <Link
          href="/"
          className="inline-flex items-center gap-2 text-base font-medium px-4 py-2 rounded-full transition-all duration-200 group hover:bg-[rgba(212,168,83,0.1)] border border-transparent hover:border-[rgba(212,168,83,0.3)]"
          style={{ color: "var(--text-body)" }}
        >
          <svg className="w-5 h-5 group-hover:translate-x-1 transition-transform text-gold" style={{ color: "var(--gold)" }} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
      ...
      <header className="relative z-10 max-w-3xl mx-auto px-4 pt-6 pb-10">
        ...
        <div className="flex flex-wrap gap-3 mb-6">
          {[
            ...
            <span
              key={i}
              className="inline-flex items-center gap-2 text-sm px-6 py-2.5 rounded-full font-medium shadow-sm"
              style={{
            ...
            >
              <span className="text-sm" style={{ color: "var(--gold)" }}>{item.icon}</span>
              <span>{item.text}</span>
            </span>
          ))}
        </div>
        ...
        {episode.content.length === 0 ? (
          ...
          <div className="space-y-5 anim-up d-2">
            ...
            {[
              ...
              <p key={i} className="text-lg leading-loose" style={{ color: "var(--text-body)", paddingRight: "20px" }}>{t}</p>
            ))}
            ...
            <p className="text-lg leading-loose" style={{ color: "var(--text-body)", paddingRight: "20px" }}>
...
function Block({ block }: { block: ContentBlock }) {
  ...
  if (block.type === "info") {
    return (
      <div className="flex flex-col sm:flex-row sm:items-start gap-2 sm:gap-4 text-base md:text-lg py-4 px-6 rounded-xl bg-[rgba(255,255,255,0.03)] border border-[rgba(255,255,255,0.06)] mb-6">
        <span className="font-bold text-gold min-w-[70px] flex-shrink-0">{block.label}:</span>
